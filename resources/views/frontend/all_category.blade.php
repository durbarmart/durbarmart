            ...
            @foreach ($categories as $key => $category)
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-6">
                                ...
                                <ul class="mb-3">
                                    ...
                                    <li class="w-100"><a href="{{ route('products.collectionSlug', $subsubcategory->slug) }}" >{{ __($subsubcategory->name) }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                            @endforeach
                ...
                <div class="mb-3 bg-white">
                    <div class="sub-category-menu @php if($key < 12) echo 'active'; @endphp" id="{{ $key }}">
                        <h3 class="category-name border-bottom pb-2"><a href="{{ route('products.collectionSlug', $category->slug) }}" >{{ __($category->name) }}</a></h3>
                        <div class="row">
                            @foreach ($category->subcategories as $key => $subcategory)
                            <div class="col-lg-4 col-6">
                                <h6 class="mb-3"><a href="{{ route('products.collectionSlug', $subcategory->slug) }}">{{ __($subcategory->name) }}</a></h6>
                                <ul class="mb-3">
                                    @foreach ($subcategory->subsubcategories as $key => $subsubcategory)
                                    <li class="w-100"><a href="{{ route('products.collectionSlug', $subsubcategory->slug) }}" >{{ __($subsubcategory->name) }}</a></li>
                                    @endforeach
