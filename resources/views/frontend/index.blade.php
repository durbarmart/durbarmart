    ...
    <section class="home-banner-area">
        <div class="container">
            <div class="row no-gutters position-relative">
                <div class="col-lg-3 position-static order-2 order-lg-0">
                    <div class="category-sidebar">
                        ...
                        <ul class="categories no-scrollbar">
                            ...
                            @php
                                $categories = \App\Category::where('type','Category')->where('top', 1)->orderBy('id','desc')->get()->take(11);
                                if($categories->count() == 0){
                                    $categories = \App\Category::where('type','Category')->orderBy('id','desc')->get()->take(11);
                                }
                            @endphp
                            @foreach ($categories as $key => $category)
                                ...
                                <li class="category-nav-element" data-id="{{ $category->id }}">
                                    <a href="{{ route('products.collectionSlug', $category->slug) }}">
                                        @if($category->icon != '')
                ...
                @endphp --}}
{{-- @if($num_todays_deal > 0) col-lg-9 @else @endif --}}
    ...
    <section id="category" class="d-md-block d-none">
      <div class="container">
        <div class="grid-container slick-slider">
            @foreach (\App\Category::where('type', 'Category')->where('featured', 1)->get() as $key => $category)
