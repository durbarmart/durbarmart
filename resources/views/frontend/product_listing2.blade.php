                ...
                <ul class="breadcrumb">
                    ...
                    @if(isset($category_id))
                        @if (\App\Category::find($category_id)->slug!=null)
                            <li class="active"><a href="{{ route('products.collectionSlug', \App\Category::find($category_id)->slug) }}">{{ \App\Category::find($category_id)->name }}</a></li>
                        @endif
                    @endif
                    {{-- @php
                        dd($category_id);
                    ...
                    @if(isset($subcategory_id) && !empty($subcategory_id))
                        <li><a href="{{ route('products.collectionSlug', \App\SubCategory::find($subcategory_id)->category->slug) }}">{{ \App\SubCategory::find($subcategory_id)->category->name }}</a></li>
                        
                        <li class="active"><a href="{{ route('products.collectionSlug', \App\SubCategory::find($subcategory_id)->slug) }}">{{ \App\SubCategory::find($subcategory_id)->name }}</a></li>
                    @endif
                    @if(isset($subsubcategory_id) && !empty($subsubcategory_id))

                        <li><a href="{{ route('products.collectionSlug', \App\SubSubCategory::find($subsubcategory_id)->subcategory->category->slug) }}">{{ \App\SubSubCategory::find($subsubcategory_id)->subcategory->category->name }}</a></li>
                        
                        <li><a href="{{ route('products.collectionSlug', \App\SubsubCategory::find($subsubcategory_id)->subcategory->slug) }}">{{ \App\SubsubCategory::find($subsubcategory_id)->subcategory->name }}</a></li>
                        <li class="active"><a href="{{ route('products.collectionSlug', \App\SubSubCategory::find($subsubcategory_id)->slug) }}">{{ \App\SubSubCategory::find($subsubcategory_id)->name }}</a></li>
                    @endif
                </ul>
            </div>
        </div>
                            ...
                            <div class="box-content">
                                <div class="category-filter">
                                    <ul>
                                        ...
                                        <li class=""><a href="{{ route('products.collectionSlug', $category->slug) }}">{{ __($category->name) }}</a></li>
                                        @endif
                                        @endforeach
                                        @endif
                                        @if(isset($category_id))
                                        <li class="active"><a href="{{ route('products') }}">{{__('All Categories')}}</a></li>
                                        <li class="active"><a href="{{ route('products.collectionSlug', \App\Category::find($category_id)->slug) }}">{{ __(\App\Category::find($category_id)->name) }}</a></li>
                                        @foreach (\App\Category::find($category_id)->subcategories as $key2 => $subcategory)
                                        <li class="child"><a href="{{ route('products.collectionSlug', $subcategory->slug) }}">{{ __($subcategory->name) }}</a></li>
                                        @endforeach
                                        @endif
                                        @if(isset($subcategory_id))
                                        <li class="active"><a href="{{ route('products') }}">{{__('All Categories')}}</a></li>
                                        <li class="active"><a href="{{ route('products.collectionSlug', \App\Category::find($subcategory_id)->category->slug) }}">{{ __(\App\Category::find($subcategory_id)->category->name) }}</a></li>
                                        <li class="active"><a href="{{ route('products.collectionSlug', \App\Category::find($subcategory_id)->slug) }}">{{ __(\App\Category::find($subcategory_id)->name) }}</a></li>
                                        @foreach (\App\Category::find($subcategory_id)->subsubcategories as $key3 => $subsubcategory)
                                        <li class="child"><a href="{{ route('products.collectionSlug', $subsubcategory->slug) }}">{{ __($subsubcategory->name) }}</a></li>
                                        @endforeach
                                        @endif
                                        @if(isset($subsubcategory_id))
                                        <li class="active"><a href="{{ route('products') }}">{{__('All Categories')}}</a></li>
                                        <li class="active"><a href="{{ route('products.collectionSlug', \App\Category::find($subsubcategory_id)->subcategory->category->slug) }}">{{ __(\App\Category::find($subsubcategory_id)->subcategory->category->name) }}</a></li>
                                        <li class="active"><a href="{{ route('products.collectionSlug', \App\Category::find($subsubcategory_id)->subcategory->slug) }}">{{ __(\App\Category::find($subsubcategory_id)->subcategory->name) }}</a></li>
                                        <li class="current"><a href="{{ route('products.collectionSlug', \App\Category::find($subsubcategory_id)->slug) }}">{{ __(\App\Category::find($subsubcategory_id)->name) }}</a></li>
                                        @endif
                                    </ul>
                                </div>
                            </div>
                ...
                <div class="col-xl-9">
                    <!-- <div class="bg-white"> -->
                        @if (isset($category_link))
                            @php
                                if($category_link->type == 'Category'){
                                    $type = 'category';
                                }elseif($category_link->type == 'SubCategory'){
                                    $type = 'subcategory';
                                }elseif($category_link->type == 'SubSubCategory'){
                                    $type = 'subsubcategory';                            
                                }elseif($category_link->type == 'SubSubSubCategory'){
                                    $type = 'subsubsubcategory';                            
                                }elseif($category_link->type == 'SubSubSubSubCategory'){
                                    $type = 'subsubsubsubcategory';                            
                                }elseif($category_link->type == 'SubSubSubSubSubCategory'){
                                    $type = 'subsubsubsubsubcategory';                            
                                }elseif($category_link->type == 'SubSubSubSubSubSubCategory'){
                                    $type = 'subsubsubsubsubsubcategory';                            
                                }
                            @endphp
                            <input type="hidden" name="{{$type}}" value="{{$category_link->slug}}">
