        ...
        <table class="table table-striped res-table mar-no" cellspacing="0" width="100%">
            ...
            <tbody>
                @foreach($subcategories as $key => $subcategory) 
                    @if ($subcategory->parentItem != null)
                        <tr>
                            <td>{{ ($key+1) + ($subcategories->currentPage() - 1)*$subcategories->perPage() }}</td>
                            <td><a href="{{route('products.collectionSlug',$subcategory->slug)}}" target="_blank">{{__($subcategory->name)}}</a></td>
                            <td><a href="{{route('products.collectionSlug',$subcategory->parentItem->slug)}}" target="_blank">{{$subcategory->parentItem->name}}</a></td>
