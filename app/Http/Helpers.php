...
if (! function_exists('colsFromArray2')) {
    function colsFromArray2($array, $filterKeys) {
        $newArr = [];
        $zxcv = [];
        $count = 0;
        function display($arrays,$filterKeys,$newArr2,$count = 0) {  
            foreach($arrays as $a => $val) {   
            $count++;
            echo $count;
            echo '<pre>';
            print_r($arrays);
            ...
            $branch = array(); 
                foreach($filterKeys as $keys) {
                    // if(!(isset($newArr2[$val['id']]) && empty($newArr2[$val['id']]))){
                        $newArr2[$val['id']][$keys]= '';
                    // }
                } 
            
                // array_push($newArr[$val->id][$filterKeys],$val[$filterKeys]);
                if(isset($val['parent_item_with_parent']) && !empty($val['parentItemWithParent'])){
                    echo '9sdfgsfgh';
                    display($val['parent_item_with_parent'],$filterKeys,$newArr2,$count); 
                }
            return $newArr2;
            }  
            // array_push($zxcv,$newArr);
            // $zxcv = [];
        }   
        $data = display($array,$filterKeys,$newArr);
    dd($data);
        return $newArr;
    }
}
...
if (! function_exists('colsFromArray')) {
    function colsFromArray($array, $filterKeys) {
        ...
        foreach($filterKeys as $filterKey) {
                array_push($newArr,$val[$filterKey]);
            //   $newArr[$filterKey] = $val[$filterKey];
        if(isset($val['sub_categories_with_sub_sub']) && !empty($val['sub_categories_with_sub_sub'])){
                foreach($val['sub_categories_with_sub_sub'] as $val_2) {
                    array_push($newArr,$val_2[$filterKey]);
                    
                    if(isset($val_2['subsub_categories_with_sub_sub_sub']) && !empty($val_2['subsub_categories_with_sub_sub_sub'])){
                        // dd($val_2['subsub_categories_with_sub_sub_sub']);
                        foreach($val_2['subsub_categories_with_sub_sub_sub'] as $val_3) {
                            array_push($newArr,$val_3[$filterKey]);
                            if(isset($val_3['subsubsub_categories_with_sub_sub_sub_sub']) && !empty($val_3['subsubsub_categories_with_sub_sub_sub_sub'])){
                                foreach($val_3['subsubsub_categories_with_sub_sub_sub_sub'] as $val_4) {
                                    array_push($newArr,$val_4[$filterKey]);
                                }
...
if (! function_exists('colsFromArray3')) {
    function colsFromArray3($array, $filterKeys) {
        $newArr = [];
        foreach($array as $val) {
        $element = [];
        foreach($filterKeys as $filterKey) {
            if(!(isset($newArr[$val['id']]))){
                ...
                array_push($newArr[$val['id']],$val[$filterKey]);
            //   $newArr[$filterKey] = $val[$filterKey];
        if(isset($val['parent_sub_sub_sub_with_sub_sub']) && !empty($val['parent_sub_sub_sub_with_sub_sub'])){
                // foreach($val['parent_sub_sub_sub_with_sub_sub'] as $val_2) {
                    // dd($val['parent_sub_sub_sub_with_sub_sub']);
                    array_push($newArr[$val['id']],$val['parent_sub_sub_sub_with_sub_sub'][$filterKey]);
                    
                    if(isset($val_2['parent_sub_sub_with_sub']['parent_sub_sub_with_sub']) && !empty($val_2['parent_sub_sub_with_sub']['parent_sub_sub_with_sub'])){
                        // foreach($val['parent_sub_sub_with_sub'] as $val_3) {
                            array_push($newArr[$val['id']],$val_2['parent_sub_sub_with_sub']['parent_sub_sub_with_sub'][$filterKey]);
                            if(isset($val_2['parent_sub_sub_with_sub']['parent_sub_sub_with_sub']['parent_sub_with_cat']) && !empty($val_2['parent_sub_sub_with_sub']['parent_sub_sub_with_sub']['parent_sub_with_cat'])){
                                // foreach($val_3['parent_sub_with_cat'] as $val_4) {
                                    array_push($newArr[$val['id']],$val_2['parent_sub_sub_with_sub']['parent_sub_sub_with_sub'][$filterKey]);
                                // }
...
if(!function_exists('truncate')){
    function truncate($model){
        $model1 = app(ucfirst($model));

        $model->truncate();
        // "\\App\\".$model::query()->truncate();
    }
}
...
if (! function_exists('filter_products')) {
    function filter_products($products) {
        ...
        if(BusinessSetting::where('type', 'vendor_system_activation')->first()->value == 1){
            // $aaa =  $products->where('published', '1')->orderBy('created_at', 'desc')->where(function($p) use ($verified_sellers){
            //     $p->where('added_by', 'admin')->orWhere(function($q) use ($verified_sellers){
            //         $q->whereIn('user_id', $verified_sellers);
            //     });
            // });
            $aaa =  $products->where('published', '1')
