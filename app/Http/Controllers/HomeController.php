...
// dd('asdf');
            ...
            switch ($sort_by) {
                ...
                case '3':
                    $products->selectRaw('*,case 
                                    ...
                                    when discount_type = null then unit_price
    ...
    public function search(Request $request)
    {
        ...
        $subcategory_id = (Category::where('slug', $request->subcategory)->first() != null) ? Category::where('slug', $request->subcategory)->first()->id : null;
        $subsubcategory_id = (Category::where('slug', $request->subsubcategory)->first() != null) ? Category::where('slug', $request->subsubcategory)->first()->id : null;
        $subsubsubcategory_id = (Category::where('slug', $request->subsubsubcategory)->first() != null) ? Category::where('slug', $request->subsubsubcategory)->first()->id : null;
        $subsubsubsubcategory_id = (Category::where('slug', $request->subsubsubsubcategory)->first() != null) ? Category::where('slug', $request->subsubsubsubcategory)->first()->id : null;
        ...
        if($sort_by != null){
            switch ($sort_by) {
                ...
                case '3':
                    $products->selectRaw('*,case 
                                    when discount_type = "amount" then (unit_price - discount)
                                    when discount_type is null then unit_price
                                    when discount_type = "percent" then (unit_price - (unit_price * (discount/100)))
                                    end as unit_price2');
                    $products->orderBy('unit_price2', 'asc');
        ...
        // dd($non_paginate_products,'zxcv123');
        //Attribute Filter
        ...
        $brands = ($products)->where('brand_id','!=',null)->get('brand_id');
// dd('qq'); 
        $products = ($products)->paginate(32)->appends(request()->query());
        // dd($products);

        return view('frontend.product_listing', compact('products', 'query', 'category_id', 'subcategory_id', 'subsubcategory_id', 'subsubsubcategory_id','subsubsubsubcategory_id','brand_id', 'sort_by', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color','location_id','brands'));
    }
    catch(Exception $e){
        dd($e);
    }
...
// dd($a); 
    ...
    public function collectionSlug(Request $request,$subcat)
    {
        $query = $request->q;
        // dd($subcat);
        $brand_id = '';
        ...
        $category_id = (Category::where('slug', $subcat)->first() != null) ? Category::where('slug', $subcat)->first()->id : null;
        
        $category_link = '';
        if($category_id != null){
            $category_link = Category::where('slug', $subcat)->first();
            $cat_2 = Category::where('slug', $subcat)->first();
            if($cat_2->type != 'BrandCategory'){
                $get_children = Category::where('parent',$cat_2->id)->with('subCategoriesWithSubSub')->get()->toArray();
                $get_children_ids = colsFromArray($get_children,['id']); 
        // dd($get_children);           
                $get_children_ids[] = $cat_2->id;
            }else{
                $brand_id = $cat_2->brand_id;            
                $get_children_ids[] = $cat_2->parent;
            }
            // dd($get_children);
            // $filterd_ids = array_column($get_children, ['id','']);
        ...
        if($category_id != null){
            // $conditions = array_merge($conditions, ['id' => $category_id]);
            // $conditions = array_merge($conditions, ['id' => $get_children_ids]);
            if($cat_2->type != 'BrandCategory'){
                
                // $products = Product::whereIn('main_id',$get_children_ids);
                // if(!empty($cat_2)){
                    if($cat_2->type  == 'Category'){
                        $products = Product::whereIn('category_id',$get_children_ids);
                    }elseif($cat_2->type  == 'SubCategory'){
                        $products = Product::whereIn('subcategory_id',$get_children_ids);
                    }elseif($cat_2->type  == 'SubSubCategory'){
                        $products = Product::whereIn('subsubcategory_id',$get_children_ids);
                    }elseif($cat_2->type  == 'SubSubSubCategory'){
                        $products = Product::whereIn('subsubsubcategory_id',$get_children_ids);
                    }elseif($cat_2->type  == 'SubSubSubSubCategory'){
                        $products = Product::whereIn('subsubsubsubcategory_id',$get_children_ids);
                    }elseif($cat_2->type  == 'SubSubSubSubSubCategory'){
                        $products = Product::whereIn('subsubsubsubsubcategory_id',$get_children_ids);
                    }elseif($cat_2->type  == 'SubSubSubSubSubSubCategory'){
                        $products = Product::whereIn('subsubsubsubsubsubcategory_id',$get_children_ids);
                    }
                // }else{
                //     $products = Product::whereIn('main_id',$get_children_ids);
                ...
                $products = Product::where(['category_id' => $get_children_ids, 'brand_id' => $brand_id]);
            }
        }
        // dd($products->get()->toArray(),$brand_id);
        
        // dd($subcat,$cat_2,$get_children,$get_children_ids,$products->get());
        if($min_price != null && $max_price != null){
            $products = $products->where('unit_price', '>=', $min_price)->where('unit_price', '<=', $max_price);
        }
        ...
        // dd($products->get()->toArray());
        $brands = ($products)->where('published',1)->get('brand_id');
        // dd(DB::getQueryLog(),$products,$products->get()->toArray(),$get_children_ids,$brand_id,$cat_2);
        ...
        //     echo '1';
        //     echo $product->unit_price2.'<br>';
        // }
        // die;
// dd($products->count()); 
        return view('frontend.product_listing2', compact('category_link','products', 'query', 'category_id', 'brand_id', 'sort_by', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color','location_id','brands'));
    }
